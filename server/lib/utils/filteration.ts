...
import { RecordInterface, UserInterface } from '../models/user.models';
...
//    return filterList;
// };
function filterSingle(query: Partial<RecordInterface>, record: EndpointsResponse[Endpoints.assets][0]) {
   if (
      query.date != null &&
      record.data != null &&
      new Date(query.date).getTime() == new Date(((record.data as unknown) as UserInterface).date!).getTime()
   )
      return false;
...
export function filterRecords(
   records: EndpointsResponse[Endpoints.assets],
   query: Partial<RecordInterface & { key: string[] }>
) {
